<?php
	...
	class PaginasModel extends Mysql
	{
		...
		{
			$sql = "SELECT idpost, titulo, ruta, status, DATE_FORMAT(datecreate, '%d/%m/%Y') AS fecha FROM post WHERE status != 0 ";
		...
		public function deletePagina(int $idpagina)
		{
			...
			$query = "DELETE FROM post WHERE idpost = $this->intIdPagina";
